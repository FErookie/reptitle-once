...
const base = 'http://dbji.dlut.edu.cn';
const firstPage = '/xygg/xygg.htm'; //新闻首页
                ...
                if (err) {
                    ...
                    html.find('a.Next').each((index, element) => {
                        if (element.children[0].data === '下页') {
                            data.push({
                                next: base + '/xygg/xygg/' + /[\s\S]{0,200}([0-9]{1,10}.htm)$/.exec(element.attribs.href)[1]
                            })
                        }
                    });
                    resolve(data);
        ...
        request
            ...
            .end((err, res) => {
                if (err) {
                    ...
                    let dataHead = header.find('p')[0].children[0].data;
                    //let array = /^时间：([0-9]{4}-[0-9]{2}-[0-9]{2})\s作者：([^z]{0,100})$/.exec(header.find('p')[0].children[0].data);
                    let begin = dataHead.indexOf(':');
                    value.dateStr = dataHead.substring(0 , begin + 2);
                    value.from = dataHead.substring(begin + 2 , dataHead.length);
                    value.url = data.url;
                    value.time = new Date();
